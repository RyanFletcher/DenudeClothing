...
<table cellpadding="50" width="100%" height="100%" align="center">
      ...
      <tr>
       <td colspan="6">
        ...
        <table cellpadding="15" cellspacing="0" border="0" width="930" align="center">
         <tr id="Menu" style="border-top:none;" align="center">      	   <td width="338" id="Menu" style="border-bottom: 1px solid #CCC;"><a href="../Users/index.asp">USERS</a></td>
           ...
           <td width="365" id="Menu" style="border-bottom: 1px solid #CCC;"><a href="index.asp">ITEMS</a></td>
                  ...
                  <% ItemIDVar = Request.QueryString("ItemID")
				    SourceVar = "SELECT * FROM Items WHERE ItemID =" & ItemIDVar
				       Call OpenRecordSet(GetItem,0,SourceVar)
						 ItemNameVar = GetItem.Fields("ItemName")
						 UnitPriceVar = GetItem.Fields("UnitPrice")
						 ImageSourceVar = GetItem.Fields("ImageSource")
						 GetItem.Close()
				 %>
                 <br />
         ...
         <input placeholder="Item Name" name="ItemName" type="text" value="<%= ItemNameVar %>"><br />
         <input placeholder="Unit Price" name="UnitPrice" type="text" value="<%= UnitPriceVar %>"><br />
         <input placeholder="Image Source" name="ImageSource" type="text" value="<%= ImageSourceVar %>"><br />
         <select name="ItemTypeID">
         <% SourceVar = "SELECT * FROM ItemTypes"
		 		Call OpenRecordSet(GetItemTypes,0,SourceVar)
						While NOT GetItemTypes.EOF
						ItemTypeIDVar = GetItemTypes.Fields("ItemTypeID")
						ItemTypeNameVar = GetItemTypes.Fields("ItemType") %>
			<option value="<%= ItemTypeIDVar %>"><%= ItemTypeNameVar %></option>	
            <% GetItemTypes.MoveNext()
			Wend
		GetItemTypes.Close()
		%>
        </select><br />	
         Available?<input type="checkbox" value="1" name="Available" />	<br />
         <input type="submit" value="Update Item" />
         <input name="ItemID" type="hidden" value="<%= ItemIDVar %>" />
		</form>
        <div align="right">
        <form method="post" action="Delete-Process.asp?ItemID=<%= ItemIDVar %>" name="DeleteItem">
 ...
 <title>Dènudè Clothing > Management > Items > Edit</title>
 <!-- InstanceEndEditable -->
       </td>
      </tr>  
     </table>
