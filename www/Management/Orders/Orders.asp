...
<% DuplicatedUserVar = Request.QueryString("DuplicatedUser")
   WrongEmailVar = Request.QueryString("EmailAddress")
       ...
       <td colspan="6">
        ...
        <table cellpadding="15" cellspacing="0" border="0" width="930" align="center">
         <tr id="Menu" style="border-top:none;" align="center">      	   <td width="338" id="Menu" style="border-bottom: 1px solid #CCC;"><a href="../Users/index.asp">USERS</a></td>
           <td width="137" id="Menu" style="border-bottom: 1px solid #CCC;"><a href="index.asp">ORDERS</a></td>
           ...
           <tr>
           <td height="28">OrderID</td><td>Ordered By</td><td>Date Ordered</td><td>Order Status</td>
           </tr>
           <% SourceVar = "SELECT * FROM Orders INNER JOIN Users ON Orders.UserID = Users.UserID INNER JOIN OrderStatuses ON Orders.OrderStatusID = OrderStatuses.OrderStatusID ORDER BY DateOrdered"
		   		Call OpenRecordSet(GetOrders,0,SourceVar)
					If GetOrders.EOF Then
					%>
                    <tr>
                    <td height="20" colspan="6">No Orders have been placed</td>
                    </tr>
                    <% Else 
					While NOT GetOrders.EOF
						OrderIDVar = GetOrders.Fields("OrderID")
						UserIDVar = GetOrders.Fields("UserID")
						UserNameVar = GetOrders.Fields("FirstName") & " " & GetOrders.Fields("Surname")
						OrderStatusIDVar = GetOrders.Fields("OrderStatusID")
						OrderStatusVar = GetOrders.Fields("OrderStatus")
						DateOrderedVar = GetOrders.Fields("DateOrdered")
						%>
            <tr style="border-bottom: sold 1px #000;">
             <td height="30"><a href="View.asp?OrderID=<%= OrderIDVar %>"><%= OrderIDVar %></a></td><td><%= UserNameVar %></td><td><%= DateOrderedVar %></td><td><%= OrderStatusVar %></td>
            </tr>
