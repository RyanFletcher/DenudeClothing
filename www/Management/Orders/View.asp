...
<% DuplicatedUserVar = Request.QueryString("DuplicatedUser")
   ...
   OrderIDVar = Request.QueryString("OrderID")
      ...
      SourceVar = "SELECT * FROM Orders WHERE OrderID =" & OrderIDVar
   		Call OpenRecordSet(GetOrder,0,SourceVar)
				UserIDVar = GetOrder.Fields("UserID")
				GetOrder.Close()
	  SourceVar = "SELECT * FROM Users WHERE UserID = " & UserIDVar
	  	Call OpenRecordSet(GetUser,0,SourceVar)
				OrderedByVar = GetUser.Fields("FirstName") & " " & GetUser.Fields("Surname")
				GetUser.Close() %>
        ...
        <table cellpadding="15" cellspacing="0" border="0" width="930" align="center">
         <tr id="Menu" style="border-top:none;" align="center">      	   <td width="338" id="Menu" style="border-bottom: 1px solid #CCC;"><a href="../Users/index.asp">USERS</a></td>
           ...
           <table border="0" width="100%" height="172">
                <tr>
           	 	<td height="26">Order from <a href="../Management-Users-Information.asp?UserID=<%= UserIDVar %>"><%= OrderedByVar %></a></td><td colspan="3" align="right"><a href="Print.asp?OrderID=<%= OrderIDVar %>">Print Order</a></td>
                </tr>
                <br />
                <br />
                <tr>
                <td height="20">ORDER ID</td><td>UNIT PRICE</td><td>QUANTITY</td><td>TOTAL</td>
                </tr>
           <%
		    SourceVar = "SELECT * FROM OrderItems WHERE OrderID = " & OrderIDVar
				Call OpenRecordSet(GetOrderItems,0,SourceVar)
					While NOT GetOrderItems.EOF
						ItemIDVar = GetOrderItems.Fields("ItemID")
						ItemNameVar = GetOrderItems.Fields("ItemName")
						UnitPriceVar = GetOrderItems.Fields("UnitPrice")
						QuantityVar = GetOrderItems.Fields("Quantity")
						%>
                <tr>
                <td height="27"><%= ItemNameVar %></td><td><%= UnitPriceVar %></td><td><%= QuantityVar %></td>
                </tr>
                
                <% GetOrderItems.MoveNext()
				Wend
