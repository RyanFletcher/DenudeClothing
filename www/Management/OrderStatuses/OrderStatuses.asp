       ...
       <td colspan="6">
        ...
        <table cellpadding="15" cellspacing="0" border="0" width="930" align="center">
         <tr id="Menu" style="border-top:none;" align="center">      	   <td width="338" id="Menu" style="border-bottom: 1px solid #CCC;"><a href="../Users/index.asp">USERS</a></td>
           ...
           <tr>
           <td height="28">Order Status ID</td><td>Status Name</td><td>Email to</td>
           </tr>
           <% SourceVar = "SELECT * FROM OrderStatuses"
		   		Call OpenRecordSet(GetOrderStatuses,0,SourceVar)
					If GetOrderStatuses.EOF Then
					%>x
                    <tr>
                    <td height="20" colspan="6">No Configured Order Statuses</td>
                    </tr>
                    <% Else 
					While NOT GetOrderStatuses.EOF
						OrderStatusIDVar = GetOrderStatuses.Fields("OrderStatusID")
						OrderStatusVar = GetOrderStatuses.Fields("OrderStatus")
						EmailVar = GetOrderStatuses.Fields("EmailAddressRedirection")
						%>
            <tr style="border-bottom: sold 1px #000;">
             <td height="30"><a href="Edit.asp?OrderStatusID=<%= OrderStatusIDVar %>"><%= OrderStatusIDVar %></a></td><td><%= OrderStatusVar %></td><td><%= EmailVar %></td>
            </tr>
            
            <% GetOrderStatuses.MoveNext()
			Wend
		GetOrderStatuses.Close()
	End If
		%> 
           </table>
