...
<html xmlns="http://www.w3.org/1999/xhtml"><!-- InstanceBegin template="/Templates/DenudeTemplate.dwt.asp" codeOutsideHTMLIsLocked="false" --><head>
 ...
 <title>Dènudè > Checkout > Order Summary</title>
 <!-- InstanceEndEditable --> 
		    ...
		    Call OpenRecordSet(BasketInfo,0,SourceVar)
             ...
             <form name="LogOut" id="LogOut" action="../../LogOut/LogOut-Process.asp">Logged in as <% If Session("GuestID") <> "" Then Response.Write("a Guest") Else Response.Write(Session("FirstName")) & " " & Session("Surname") End If %><% If Session("GuestID") <> "" Then
			     Else %>  (<a href="../../Profile/index.asp">Edit Profile</a>)<% End If %><br />
                  <a href="Basket.asp">Your Basket</a>: £<% If TotalOrderValueVar = "" Then %>0.00<% Else %><%= TotalOrderValueVar %>.00
       ...
       <td colspan="6">
        ...
        <table border="0" width="100%" height="400">
         <tr>
          <td align="center" width="49%">
           <table border="0" width="100%" height="400">
           <tr><td colspan="4"><p align="center" id="title">Order Summary</p></td></tr>
           <% SourceVar = "SELECT Basket.ItemID, Basket.UserID, Basket.Quantity, Items.ItemID, Items.ItemName, Items.UnitPrice, Items.ImageSource FROM Basket INNER JOIN Items ON Basket.ItemID = Items.ItemID WHERE UserID =" & Session("UserID") &"" 
		   Call OpenRecordSet(ViewBasket,0,SourceVar)
				While NOT ViewBasket.EOF
		   		ItemIDVar = ViewBasket.Fields("ItemID")
				ItemNameVar = ViewBasket.Fields("ItemName")
				UnitPriceVar = ViewBasket.Fields("UnitPrice")
				ImageSourceVar = ViewBasket.Fields("ImageSource")
				QuantityVar = ViewBasket.Fields("Quantity") %>
                <tr>
                 <td align="center" width="16%">
				   <img width="150" height="150" src="../../Images/<%= ImageSourceVar %>" name="<%= ItemNameVar %>" /></td>
                  <td id="FormLabel" width="84%">
                   <p>Item Name: <%= ItemNameVar %></p>
                   <p>Unit Price: £<%= UnitPriceVar %></p>
                   <p>Quantity: <%= QuantityVar %>
                </td>
               </tr> 
            <% ViewBasket.MoveNext()
		     Wend
		   ...
		   SourceVar = "SELECT SUM(Items.UnitPrice * Basket.Quantity) AS Total,Basket.UserID FROM Basket INNER JOIN Items ON Basket.ItemID = Items.ItemID GROUP BY Basket.UserID"
		...
		TotalOrderValue.Close()
          ...
          If TotalOrderValueVar <> "" Then %>
           <tr>
            <td colspan="2"><p>Total: £<%= TotalOrderValueVar %></p></td>
           </tr>
           <tr>
            <td><form method="post" action="Basket-Checkout-Process.asp" name="ProcessOrder" id="ProcessOrder"><input type="submit" value="Place Order" /></form></td></tr>
            <% End If %>
           </table>     
          </td> 
         </tr>
