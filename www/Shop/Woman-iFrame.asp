...
<body>
           ...
           <tr align="center">
           <%
		     SourceVar = "SELECT ItemID,ItemTypeID,ItemName,UnitPrice,ImageSource, Available FROM Items WHERE ItemTypeID = 2"
           ...
           <form name="Item<%= ItemIDVar %>"  action="Checkout/Basket-Add-Process.asp?ItemID=<%= ItemIDVar %>&amp;ItemName=<%= ItemNameVar %>&amp;UnitPrice=<%= UnitPriceVar %>" method="post">
			...
			End If %></p><img width="170" height="170" src="../Images/<%= ImageSourceVar %>" name="<%= ItemNameVar %>" />
