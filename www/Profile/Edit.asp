...
<title>Dènudè > Profile > Edit</title>
 <!-- InstanceEndEditable --> 
		...
		SourceVar = "SELECT SUM(Items.UnitPrice * Basket.Quantity) AS Total, SUM(Basket.Quantity) AS Quantity,Basket.GuestID FROM Basket INNER JOIN Items ON Basket.ItemID = Items.ItemID WHERE GuestID = " & Session("GuestID") & " GROUP BY Basket.GuestID"
		    ...
		    Call OpenRecordSet(BasketInfo,0,SourceVar)
             ...
             <form name="LogOut" id="LogOut" action="../LogOut/LogOut-Process.asp">Logged in as <% If Session("GuestID") <> "" Then Response.Write("a Guest") Else Response.Write(Session("FirstName")) & " " & Session("Surname") End If %><% If Session("GuestID") <> "" Then
			     Else %>  (<a href="index.asp">Edit Profile</a>)<% End If %><br />
      ...
      <tr>
       <td colspan="6">
        ...
        <table border="0" width="100%" height="400">
         <tr>
         <td width="25%"></td>
         <td style="border-right: 1px solid #CCC; border-left: 1px solid #CCC;" align="center"><p id="title">Your Profile</p></td>
         <td width="25%"></td>
         </tr>
         <tr>
         <td>
         </td>
          <td valign="top" style="border-right: 1px solid #CCC; border-left: 1px solid #CCC;" width="50%">
		  <% UserIDVar = Session("UserID")  
		   SourceVar = "SELECT * FROM Users WHERE UserID =" & UserIDVar
				Call OpenRecordSet(User,0,SourceVar)
					 If User.EOF Then 
					 Response.Redirect("../Index.asp")
					 End If
					 FirstNameVar = User.Fields("FirstName")
					 SurnameVar = User.Fields("Surname")
					 EmailAddressVar = User.Fields("EmailAddress")
					 PasswordVar = User.Fields("Password")
					 User.Close()
					 %>
                     <p id="FormLabel" align="center">
           <form  class="" action="Edit-Process.asp" method="post">
				    First Name<br />
  <input type="text" value="<%= FirstNameVar %>" /><br />
  					  Surname<br />
  <input type="text" value="<%= SurnameVar %>" /><br />
  					  Email Address<br />
  <input type="text" value="<%= EmailAddressVar %>" /><br />
  					  Password<br />
  <input type="password" value="<%= PasswordVar %>"  /><br />
				    Address Line1 Name<br />
  <input type="text" value="<%= AddressLine1Var %>" /><br />
  					  Address Line 2<br />
  <input type="text" value="<%= AddressLine2Var %>" /><br />
  					  Address Line 3<br />
  <input type="text" value="<%= AddressLine3Var %>" /><br />
				      Postcode<br />
  <input type="text" value="<%= PostcodeVar %>" /><br />
  					  County<br />
  <input type="text" value="<%= CountyVar %>" /><br />
  					  Country<br />
  <input type="text" value="<%= CountryVar %>" /><br />
